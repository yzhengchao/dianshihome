 ...
 * 封封微信的的request
 */
function request(url, data = {}, method, header,type) {
      // wx.showLoading({
      //   title: '加载中...',
      // });
    ...
    if(store.state.userId){
        header.userid = store.state.userId
        // header.userid = "465f806552e9606fede2938db1e92fd9"
        // console.log(store.state.userId)
    }
    if(store.state.auth){
        header.authorization = store.state.auth
    }
    if(store.state.appid){
        header.appid = store.state.appid
    }
...
*/
function arrmax(arr){
    var max = Number(arr[0].price)
    let obj =arr[0]
    if(arr.length>1){
        arr.forEach(r=>{
            if(max<Number(r.price)){
                max = Number(r.price)
                obj = r
            }
        })
